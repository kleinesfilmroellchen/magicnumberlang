...
session.headers.update({'user-agent': 'nhentai-bot'})


print("Using custom nhentai.py with session", session)
...
class Doujinshi():
    ...
    def fetch(self, res=None):
        ...
        for tag in res('a', class_='tag'):
            tagt = tag.find('span', class_='name').text
            self.tags.append(tagt)
        self.fetched = True
...
def _get(endpoint: str):
    ...
    while response.status_code == 429:
        # print("too many requests, retry after some time")
        time.sleep(random()/4)
        response = session.get(endpoint)
